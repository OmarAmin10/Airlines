...
<Grid className='grid'  container spacing={3} columns={16}>
  ...
  <select className="select border form-label"  id='aa' onChange={(event)=>setFrom(event.target.value)} >
...
{ Airport.Airport.map((result)=>(<option value={result.code}>{result.country}, {result.code} </option>))}
  ...
  <label for="aa" className="form-label">From</label>
  </Grid>

  <Grid item xs={3}>
  <select className="select border" id='a'  onChange={(event)=>setTo(event.target.value)} >
                   <option disabled="true" selected>Flying To  </option>
{ Airport.Airport.map((result)=>(<option value={result.code}>{result.country}, {result.code} </option>))}
  </select>
  <label for="a" className="form-label">To</label>   
...
<label for="fo" className="form-label">Departure Date</label>
  ...
  <Grid item xs={3}>
    ...
    <input type="time" className="form-control border" 
...
<label for="exampletimepicker1" className="form-label">Departure Time</label>
  ...
  <Grid item xs={3}>
    ...
    <input type="time"  className="form-control fo border" id="exampletimepicker1" onChange={(event) => setarrival_number(event.target.value)} value={arrival_number}
                style={{ height: "50px", width: "250px" }} />
